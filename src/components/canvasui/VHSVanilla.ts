/* VHS — Canvas UI, vanilla flavour.
 *
 * Vendored from the Canvas UI shadcn registry rather than installed as a
 * dependency, which is how the library ships: the source lands in your
 * project and you version it like anything you wrote.
 *
 *     source   https://canvasui.dev/docs/components/vhs
 *     registry npx shadcn@latest add @canvas-ui/vhs-vanilla
 *     licence  MIT + Commons Clause — free to use, not to resell
 *
 * Unmodified. src/scripts/lcd-vhs.ts is where this deck's own wiring lives,
 * so re-running the command above overwrites this file and loses nothing.
 *
 * It draws through the experimental HTML-in-canvas API — drawElementImage()
 * on a 2D context, requestPaint() on the canvas — which today means Chrome
 * with chrome://flags/#canvas-draw-element, or an origin trial token in
 * production. supportsHtmlInCanvas() is exported for exactly that reason:
 * everywhere else the readout is left as plain DOM and nothing here runs.
 */
